  ...
  return <main className="flex h-screen min-h-0 flex-col bg-gray-950 text-gray-100">
    <header className="shrink-0 border-b border-gray-800 bg-gray-900/95 px-4 py-3 shadow-lg">
      ...
      <div className="mt-3 grid grid-cols-[auto_minmax(0,1fr)_auto] gap-2">
        <button type="button" disabled={currentIndex <= 0} onClick={() => void window.electronAPI?.modelInspectorNavigate('previous')} className="inline-flex items-center gap-1 rounded-md border border-cyan-500/25 bg-gray-900 px-2.5 py-2 text-sm text-gray-200 hover:border-cyan-400/50 hover:bg-gray-800 disabled:border-gray-800 disabled:text-gray-600"><ChevronLeft className="h-4 w-4" />Previous</button>
        ...
        <button type="button" disabled={currentIndex >= snapshot.items.length - 1} onClick={() => void window.electronAPI?.modelInspectorNavigate('next')} className="inline-flex items-center gap-1 rounded-md border border-cyan-500/25 bg-gray-900 px-2.5 py-2 text-sm text-gray-200 hover:border-cyan-400/50 hover:bg-gray-800 disabled:border-gray-800 disabled:text-gray-600">Next<ChevronRight className="h-4 w-4" /></button>
      </div>
    ...
    <div className="min-h-0 flex-1 overflow-auto p-4 sm:p-5">
      ...
      {location.sourceKind === 'lora' && <section className="mb-4 rounded-xl border border-gray-800 bg-gray-900/70 p-4"><div className="flex items-center justify-between gap-3"><div><div className="text-xs uppercase tracking-wider text-gray-500">LoRA syntax</div><code className="mt-1 block break-all text-sm text-emerald-200">{getDefaultLoraSyntax(location, currentItem.localMetadata)}</code></div><button type="button" onClick={() => void copyText(getDefaultLoraSyntax(location, currentItem.localMetadata), 'Unable to copy LoRA syntax.')} className="shrink-0 rounded-md border border-gray-700 px-2.5 py-2 text-xs hover:bg-gray-800">Copy LoRA syntax</button></div></section>}
