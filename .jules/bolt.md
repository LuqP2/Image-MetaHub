...
## 2024-06-19 - Eliminate O(N*K) redundant traversals in numeric bucket calculation
**Learning:** The analytics dashboard was using multiple `.filter()` calls (one per range) in `buildNumericBuckets` to aggregate image counts. This resulted in O(N*K) property selector calls and full array traversals, which is inefficient for large datasets and multiple metric groups (generation time, speed, VRAM).
**Action:** Replace multiple `.filter()` passes with a single O(N) loop that evaluates the property once per image and increments the appropriate bucket counts. This reduces overall CPU time and memory pressure in the analytics explorer.
