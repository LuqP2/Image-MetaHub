...
**Learning:** Initializing a `Map` or `Set` using `.map()` on a large array (e.g., `new Map(arr.map(item => [item.id, item]))`) causes an O(N) temporary array of tuples to be allocated and immediately discarded, triggering garbage collection pauses.
**Action:** Replace `.map()` with a pre-instantiated `Map` or `Set` and populate it directly using a `for` loop to scale at O(1) intermediate memory.
## 2026-05-22 - Eliminate Object.entries/values/keys array allocation overhead
**Learning:** Iterating over object properties using `Object.entries()`, `Object.values()`, or `Object.keys()` inside tight loops (like graph BFS traversals) causes unnecessary O(N) temporary array allocations and limits early-returns, significantly increasing GC pressure.
**Action:** Replace `Object.*` methods with direct `for...in` loops to traverse properties. When iterating, verify object type structures directly or use the loop for early breaking instead of chaining array methods.
