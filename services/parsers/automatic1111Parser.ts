...
export function parseA1111Metadata(parameters: string): BaseMetadata {
  ...
  // Extract model - try Model: field first for A1111, then Hashes JSON for ComfyUI/other
  const modelMatch = parameters.match(/Model: ([^,]+)/);
  if (modelMatch) {
    result.model = modelMatch[1].trim();
  }
  
  // If no model from Model: field, try Hashes JSON
  if (!result.model) {
  ...
  // If still no model, try Model hash: pattern as fallback
  if (!result.model) {
