...
- **File Deletion Sync**: Fixed an issue where images deleted from a watched folder would not automatically disappear from the gallery, requiring a manual refresh.
- **Context Menu Actions**: Fixed "Open in Explorer" failing for subfolders due to case-sensitivity issues on Windows key paths.
