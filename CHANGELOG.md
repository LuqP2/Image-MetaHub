...
- **Portable Installation**: Create an empty `portable.txt` file next to the app and Image MetaHub keeps settings, tags, ratings, thumbnails, cache and logs in a `data` folder next to the installation instead of the user's application data folder, so a copy on a USB drive carries its library configuration between machines and leaves nothing behind on the host. Writing a folder path inside `portable.txt` stores the data elsewhere, and the `IMH_PORTABLE` and `IMH_PORTABLE_DATA_DIR` environment variables do the same without a marker file. Existing settings are copied over on the first portable start, and the app falls back to the standard location (explaining why in Settings → Library → App data location) if the folder cannot be written to.
