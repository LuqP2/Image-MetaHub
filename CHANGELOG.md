...
- **Portable Installation**: Settings → Library → App data location has a new "Store app data next to the app" switch. With it on, settings, tags, ratings, thumbnails, cache and logs live in a `data` folder next to the installation instead of the user's application data folder, so a copy on a USB drive carries its library configuration between machines and leaves nothing behind on the host. Existing settings are copied over the first time, and the app falls back to the standard location (explaining why in the same panel) if the folder cannot be written to. For preparing a drive before the first launch, an empty `portable.txt` file next to the app does the same thing, a folder path written inside it stores the data elsewhere, and the `IMH_PORTABLE` / `IMH_PORTABLE_DATA_DIR` environment variables override both.
